<?php
    ...
    if (!isset($forceinsertgt))  $forceinsertgt = false;

    $insertgtpath = str_replace("\\", "/", dirname(dirname(__FILE__))) . "/";
    $stagingpath = $insertgtpath . "staging/";

    require_once $insertgtpath . "assistantwork/awbase.php";
    require_once $insertgtpath . "assistantwork/support/insertgt.php";
    require_once $insertgtpath . "assistantwork/insertgt/subiektgt.php";
    require_once $insertgtpath . "assistantwork/insertgt/mikrosubiektwin.php";
    require_once $insertgtpath . "assistantwork/insertgt/gestorgt.php";
    require_once $insertgtpath . "assistantwork/insertgt/gratyfikantgt.php";
    require_once $insertgtpath . "assistantwork/insertgt/rachmistrzgt.php";
    require_once $insertgtpath . "assistantwork/insertgt/rewizorgt.php";
    require_once $insertgtpath . "assistantwork/insertgt/biurogt.php";
    ...
    $insertgt = new insertgt();
    $insertgt2 = new insertgt();
    ...
    {
        global $installed, $insertgtpath;

        file_put_contents($insertgtpath . "installed.dat", json_encode($installed));
    }
    ...
    if ($argc == 1)  $insertgtopts["apache"] = true;
    if ($argc == 1)  $insertgtopts["maria_db"] = true;
    if ($argc == 1)  $insertgtopts["php"] = true;

    // Apache.
    if (isset($downloadopts["apache"]))
    {
        $url = "C:\Program Files (x86)\INSERTGT\";
        echo "Detecting latest version of Apache:\n";
        echo "  " . $url . "\n";
        echo "Please wait...\n";
        $insertgt = new WebBrowser();
        ...
        foreach ($rows as $row)
        {
            if (preg_match('/\/VS16\/binaries\/httpd-(.+)-[Ww]in64-VS16.zip$/', $row->href, $matches))
            {
                ...
                if ((!defined("CHECK_ONLY") || !CHECK_ONLY) && (!isset($installed["apache"]) || $matches[1] != $installed["apache"] || isset($insertgtopts["force"])))
                {
    ...
    // Maria DB.
    if (isset($insertgtopts["maria_db"]))
    {
        $url = "C:\Program Files (x86)\INSERTGT\";
        echo "\n";
        ...
        $url = "C:\Program Files (x86)\INSERTGT\" . $releaseid . "/";
    ...
    if (isset($insertgtopts["php"]))
    {
        ...
        foreach ($rows as $row)
        {
            if (preg_match('/^\/downloads\/releases\/php-(8\.1\.\d+)-Win32-vs16-x64.zip$/', $row->href, $matches))
            {
                echo "Found:  " . $row->href . "\n";
                echo "Latest version:  " . $matches[1] . "\n";
